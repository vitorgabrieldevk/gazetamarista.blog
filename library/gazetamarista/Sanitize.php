...
class gazetamarista_Sanitize {
	 ...
	 * @return string
	 */
	static public function sanitizestring($texto, $tipo) {
	    /*
	    ...
	    */
		...
		if(!is_null($texto)) {
			...
			if(is_array($texto)) {
				...
				foreach($texto as $field=>$value) {
                    ...
                    if (is_array($value)) {  // Verifica se é um array
                        ...
                        foreach ($value as $field2 => $value2) {
                            ...
                            if ($tipo == "search") {
                                $value[$field2] = htmlspecialchars(addslashes(strip_tags(trim($value2))));
                            } else {
                        ...
                        // Verifica se está sendo utilizado na busca ou no insert/update
                        if ($tipo == "search") {
                            $texto[$field] = htmlspecialchars(addslashes(strip_tags(trim($value))));
                        } else {
                ...
                // Verifica se está sendo utilizado na busca ou no insert/update
                if($tipo == "search") {
                    $texto = htmlspecialchars(addslashes(strip_tags(trim($texto))));
                }else{
