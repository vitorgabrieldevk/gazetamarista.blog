...
class canvas {
     ...
     private function dados()
     {
          ...
          if ( is_file( $this->origem ) ) {
            // Imagem existente
              ...
              if($_GET["largura"] == $_GET["altura"]) {
                  $img_not_found = "not_found_img.jpg";
              }else{
                  $img_not_found = "not_found_img2.jpg";
              }
              $this->origem = APPLICATION_PATH . "/../common/uploads/default/" . $img_not_found;
    ...
    **/
     ...
     **/
     ...
     public function redimensiona( $nova_largura = 0, $nova_altura = 0, $tipo = '' )
     {
          ...
          switch( $tipo )
          {
               ...
               case 'sempreenchimento':
                    // modo crop sem preenchimento, fundo transparente
                    $this->redimensionaSempreenchimento();
                    break;
               default:
     ...
     /**
      * Redimensiona imagem, cropando para encaixar no novo tamanho, sem sobras, com fundo transparente
      * baseado no script original de Noah Winecoff
      * http://www.findmotive.com/2006/12/13/php-crop-image/
      * atualizado para receber o posicionamento X e Y e/ou Coordenadas Inteligentes
      * do crop na imagem.
      ...
      * @return void
      **/
     private function redimensionaSempreenchimento()
     {
        // Cria imagem de destino temporária
        $this->img_temp = imagecreatetruecolor( $this->nova_largura, $this->nova_altura );

        // Fundo transparente
        imagefill($this->img_temp,0,0,0x7fff0000);
     ...
     **/
     ...
     {
			$watermark_img	= APPLICATION_PATH . "/../common/default/images/marcadagua.png"; // use GIF or PNG, JPEG has no tranparency support
			$padding 		= 3; // distance to border in pixels for watermark image

			$watermark 	= imagecreatefrompng($watermark_img); // create watermark
			$image 		= $this->img; // create main graphic
			...
			imagecopymerge($image, $watermark, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height, $alfa);
