    ...
    public function menuitem($campo) {
        ...
        if(count($arrcampo) > 1) {
            ...
            if(!empty($coluna)) {
                // Select
                $select = $model->select()
                    ->from(array('menuitem' => $model->getTableName()), array('*'))
                    ->joinInner(array('menucateg' => (new Admin_Model_Menuscategorias())->getTableName()), "menuitem.idcategoria = menucateg.idcategoria", array('categoria' => 'descricao'))
