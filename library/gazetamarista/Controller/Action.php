...
class gazetamarista_Controller_Action extends Zend_Controller_Action {
        ...
        foreach($form->getElements() as $element) {
            ...
            switch($field_type) {
                ...
                case "datetime":
                    ...
                    // Verifica se a data existe
                    if($value == "00/00/0000 00:00:00" || $value == "00/00/0000") {
                        $field_value = "";
                    }
                    break;
        ...
        foreach($form->getElements() as $element) {
            ...
            switch($field_type) {
                ...
                case "datetime":
                    ...
                    // Verifica se a data existe
                    if($date == "00/00/0000 00:00:00" || trim($date) == ':') {
                        $date = "";
                    }
    ...
    protected function viewAction() {
        ...
        if(count($where) > 0) {
            ...
            // Se for tabela de contato, marcar como registro visualizado
            if($this->_request->getParam("controller") == "contatos") {
                if($id > 0) {
                    // Seta como visualizado
                    $data_upvisu = array();
                    $data_upvisu['visualizado'] = 1;
                    $this->_model->update($data_upvisu, array('idcontato = ?' => $id));
                }
            }
        }
        else {
    ...
    protected function exportpdfAction() {
        ...
        if(!empty($controlador_atual)) {
            ...
            $rowmenu = $model_menuitem->fetchRow(array('controlador = ?' => $controlador_atual));
            if($rowmenu) {
        ...
        // Se for pedidos, adiciona extras
        if($this->_model->getTableName() == "pedidos") {
            // Seleciona os produtos
            $model_pedidoprodutos = new Admin_Model_Pedidosprodutos();
            $select_produtos = $model_pedidoprodutos->select()
                ->from("pedidos_produtos", array('*'))
                ->joinInner("produtos", "produtos.idproduto = pedidos_produtos.idproduto", array('*'))
                ->where("pedidos_produtos.idpedido = ?", $idregistro)
                ->group('pedidos_produtos.idproduto')
                ->setIntegrityCheck(FALSE);

            $produtos = $model_pedidoprodutos->fetchAll($select_produtos);

            // Cabeçalho da lista de Produtos
            $page->setFont($fontB, 11)
                ->setFillColor(Zend_Pdf_Color_Html::color('#000000'))
                ->drawText(":: Lista de Produtos", 50, $y, 'UTF-8');
            ...
            foreach($produtos as $produto) {
                ...
                $qtd_prod = (int)$produto->quantidade;
                ...
                $produto_label = '' . trim($produto->descricao) . ' - ' . $produto->titulo . ' ' . $produto->colecao . ': ' . $qtd_prod;
                $page->setFont($font, 11)
                    ->setFillColor(Zend_Pdf_Color_Html::color('#000000'))
                    ->drawText($produto_label, 65, $y, 'UTF-8');
            ...
            $model_pedidohistorico = new Admin_Model_Pedidosstatus();
            $select_historicos = $model_pedidohistorico->select()
                ->from("pedidos_status", array('*'))
                ->where("pedidos_status.idpedido = ?", $idregistro)
                ->order("pedidos_status.data_execucao DESC")
                ->setIntegrityCheck(FALSE);

            $historicos = $model_pedidohistorico->fetchAll($select_historicos);
            ...
            foreach($historicos as $historico) {
                ...
                if($historico->idstatus_pedido == 1) {
                    $txt_status = "*Usuário acessou a tela de checkout";
                }else{
                    $txt_status = "*" . $historico->status_pedido;
                }

                $historico_label = date("d/m/Y H:i", strtotime($historico->data_execucao)) . ' - ' . $historico->identificacao . ' ' . $txt_status;
                $page->setFont($font, 11)
                    ->setFillColor(Zend_Pdf_Color_Html::color('#000000'))
