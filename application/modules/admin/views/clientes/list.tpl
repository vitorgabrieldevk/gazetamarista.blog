<div class="row">	
	...
	{if $esconderBtnFiltrar != true}
		<div id="filtros" class="reveal-modal" data-reveal aria-labelledby="Filtros" aria-hidden="true" role="dialog">
			<form action="{$this->url(['module'=>'admin', 'controller'=>$controller, 'action'=>'list', {key($requireParam)}, {current($requireParam)}], 'default', TRUE)}" method="post">
				<div class="row">
					...
					<div class="element-form" id="element-data_inicial">
						<div class="row">
							<div class="small-12 medium-12 large-12 columns labeldiv" id="label-data_inicial">
								<label for="data_inicial" class="optional">Data inicial <div class="clearfix"></div>
									<small>Data inicial</small>
								</label>
							</div>
							<div class="input-form small-12 medium-3 large-2 columns end">
								<input name="data_inicial" id="data_inicial" value="{if $filtro_data_inicial|default:"" != ""}{$this->dateformat("%d/%m/%Y", $filtro_data_inicial)}{/if}" field-type="date" class="datepicker radius" data-datepicker="" data-ac_label="" placeholder="Buscar pela data inicial do pedido" type="text">
							</div>
						</div>
					</div>

					<div class="element-form" id="element-data_final">
						<div class="row">
							<div class="small-12 medium-12 large-12 columns labeldiv" id="label-data_final">
								<label for="data_final" class="optional">Data final <div class="clearfix"></div>
									<small>Data final</small>
								</label>
							</div>
							<div class="input-form small-12 medium-3 large-2 columns end">
								<input name="data_final" id="data_final" value="{if $filtro_data_final|default:"" != ""}{$this->dateformat("%d/%m/%Y", $filtro_data_final)}{/if}" field-type="date" class="datepicker radius" data-datepicker="" data-ac_label="" placeholder="Buscar pela data final do pedido" type="text">
							</div>
						</div>
					</div>
				...
				<div class="row">
					...
					<div class="small-12 medium-2 end columns">
						<button type="reset" class="input-reset-search expand" data-url="{$basePath}/admin/{$currentController}/list">Limpar</button>
					</div>
				</div>
			</form>
