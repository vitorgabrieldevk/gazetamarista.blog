 ...
 */
class Admin_AvioesvendaController extends gazetamarista_Controller_Action {
	/**
	 * Armazena o model padrão da tela
	 *
	 * @access protected
	 * @name $_model
	 * @var Admin_Model_Avioesvenda
	 */
	...
	public function init() {
		...
		$this->_model = new Admin_Model_Avioesvenda();
		$this->_model_itens = new Admin_Model_Aviaovendaitens();
	...
	public function doBeforePopulate($data) {
		// Busca o id
		$id = $this->_request->getParam("idaviaovenda", 0);
	
		// Se for editar
		if($id > 0) {
			// Seleciona vantagens
            $itens_vantagens = $this->_model_itens->fetchAll(array('idaviaovenda = ?' => $id, 'tipo = ?' => 'vantagens'), 'ordenacao ASC');

            // Seleciona performances
            $itens_performances = $this->_model_itens->fetchAll(array('idaviaovenda = ?' => $id, 'tipo = ?' => 'performance'), 'ordenacao ASC');
	...
	public function doAfterInsert($id) {
        ...
        if($vantagens) {
            // Busca última ordem
            $nova_ordem = 1;
            $ordem_vantagem = $this->_model_itens->fetchRow(array('idaviaovenda = ?' => $id, 'tipo = ?' => 'vantagens'), 'ordenacao DESC');
            if($ordem_vantagem) {
        ...
        if($performances) {
            // Busca última ordem
            $nova_ordem = 1;
            $ordem_performance = $this->_model_itens->fetchRow(array('idaviaovenda = ?' => $id, 'tipo = ?' => 'performance'), 'ordenacao DESC');
	...
	public function deletaritemAction() {
		// Busca o item
        $id     = $this->_request->getParam("idaviaovenda", 0);
		$iditem = $this->_request->getParam("iditemvenda", 0);
		
		if($id > 0 && $iditem > 0) {
			// Remove o item
			$this->_model_itens->delete(array('idaviaovenda = ?' => $id, 'idaviaoitemvenda = ?' => $iditem));
		}
	...
	public function salvaordemAction() {
		...
		if($id > 0 && !empty($type) && is_array($objOrdemId)) {
			// Seleciona os itens
			$rows = $this->_model_itens->fetchAll(array('idaviaovenda = ?' => $id, 'tipo = ?' => $type));

			if(count($rows) > 0) {
				// Zera a ordenação atual
				$this->_model_itens->update(array('ordenacao' => 0), array('idaviaovenda = ?' => $id, 'tipo = ?' => $type));

				// Percorre os itens
                $cont_ordem = 1;
				foreach($objOrdemId as $iditem) {
					// Efetua o update com a nova ordenação
					$this->_model_itens->update(array('ordenacao' => $cont_ordem), array('idaviaoitemvenda = ?' => $iditem, 'idaviaovenda = ?' => $id, 'tipo = ?' => $type));
