...
class Bootstrap extends Zend_Application_Bootstrap_Bootstrap {
    ...
    protected function _initTranslate()
    {
        try
        {
			...
			$linguagem = (new Zend_Session_Namespace("linguagem"));
			...
			if( !$default_language )
			{
				// Busca a linguagem pelo header da requisição, somente a primeira parte, ex.: en-US ou en-GB retornna 'en'
				$languageClient = explode('-',explode(',', $_SERVER['HTTP_ACCEPT_LANGUAGE'] ?? '')[0] ?? '')[0] ?? '';

				switch( $languageClient ) {
					case 'es':
						$default_language = 'es';
						break;
			...
			if( !$linguagem->lingua )
			{
				$linguagem->lingua = $language_nomes[$default_language];
			}
