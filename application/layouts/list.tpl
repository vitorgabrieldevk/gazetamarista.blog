			...
			<form action="{$this->url(['module'=>'admin', 'controller'=>$controller, 'action'=>'search', {key($requireParam)}, {current($requireParam)}], 'default', TRUE)}" method="post">
				...
				<div class="row">
					...
					<div class="small-12 medium-2 end columns">
						<button type="reset" class="input-reset-search expand button secondary normal" data-url="{$basePath}/admin/{$currentController}/list" ><span class="mdi mdi-keyboard-backspace"></span> Limpar</button>
					</div>
				</div>
			</form>
