...
$(document).ready(() => {
	...
	if( $site_corpo.hasClass('main-noticias-detalhe') )
	{
		var idPost = parseInt($('#idpost').val());

		let hasLikedUnparsed = window.localStorage.getItem(`SYNCJET_POST${idPost}`);
				...
				.done(async function (data) {
					if (data.status === "erro") {
						...
						if ($(".like").hasClass("liked")) {
							...
							handleStorage("set", "SYNCJET_POST" + idPost, "like");
				...
				.done(async function (data) {
					if (data.status === "erro") {
						...
						if ($(".unlike").hasClass("disliked")) {
							...
							handleStorage("set", "SYNCJET_POST" + idPost, "unlike");
