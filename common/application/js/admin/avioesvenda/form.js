$(function() {
	...
	$(".list-vantagens").sortable({
		...
		update: function() {
			...
			$.ajax({
				url		: document.basePath + "/admin/avioesvenda/salvaordem",
				type	: 'POST',
	...
	$('a.btn-add-vantagem-venda').on('click', function(e) {
		...
		if(!!input_vantagem) {
			...
			if(existe_termo.length > 0 || existe_termo_en.length > 0 || existe_termo_es.length > 0) {
				...
				var remove 		= $('<div class="small-2 medium-1 columns"><a href="' + document.basePath + '/admin/avioesvenda/deletaritem/" title="Excluir item" class="btn-remove-vantagem-venda btn_excluir"></a></div>');
	...
	$('.list-vantagens').on('click', 'a.btn-remove-vantagem-venda', function(e) {
	...
	// Adiciona o evento de adicionar à lista
	$('a.btn-add-performance-venda').on('click', function(e) {
		// Cancela o click do anchor
		e.preventDefault();
		...
		if(!!input_performance) {
			...
			if(existe_termo.length > 0 || existe_termo_en.length > 0 || existe_termo_es.length > 0) {
				...
				var remove 		= $('<div class="small-2 medium-1 columns"><a href="' + document.basePath + '/admin/avioesvenda/deletaritem/" title="Excluir item" class="btn-remove-performance-venda btn_excluir"></a></div>');
				var item 		= $('<div class="small-10 medium-11 columns">' +
	...
	// Evento que remove os itens
	$('.list-performances').on('click', 'a.btn-remove-performance-venda', function(e) {
		if(!confirm('Deseja remover este item?')) {
			return false;
		}else{
			e.preventDefault();
